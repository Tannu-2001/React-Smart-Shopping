...
$(function () {
  ...
  try {
    ...
    // fetch products once
    const resp = await fetch('http://127.0.0.1:4400/getproducts');
    if (!resp.ok) {
...
sanitizeCart().then(() => {
    ...
    $.ajax({ method: "GET", url: "register.html" })
      .then(function (response) {
        ...
        $("#btnRegister").off("click").on("click", function () {
          ...
          $.ajax({
            method: "POST",
            url: "http://127.0.0.1:4400/customerregister",
            data: customerData
          }).then(function () {
  ...
  function UserLogin(onSuccess) {
    $("#btnLogin").off("click").on("click", function () {
      ...
      $.ajax({ method: "GET", url: "http://127.0.0.1:4400/getcustomers" })
        .then(function (response) {
  ...
  $.ajax({ method: "GET", url: "http://127.0.0.1:4400/getproducts" })
    .then(function (products) {
      ...
      $("#btnCheckout").off("click").on("click", async function(){
        ...
        try {
          const productsResp = await $.ajax({ method: "GET", url: "http://127.0.0.1:4400/getproducts" });
          ...
          const createResp = await $.ajax({
            method: "POST",
            url: "http://127.0.0.1:4400/createorder",
  ...
  function GetProducts(categoryName) {
    const url = categoryName ? `http://127.0.0.1:4400/categories/${encodeURIComponent(categoryName)}` : "http://127.0.0.1:4400/getproducts";
